...
BEGIN
    ...
    DECLARE @cmpS   INT = (SELECT TOP 1 CampaignID FROM [Notification].[Campaigns] WHERE CampaignName=N'Thông Báo Sale Hè 2026');
    DECLARE @cmpV   INT = (SELECT TOP 1 CampaignID FROM [Notification].[Campaigns] WHERE CampaignName=N'Gửi Voucher Welcome Khách Mới');
    DECLARE @cmpR   INT = (SELECT TOP 1 CampaignID FROM [Notification].[Campaigns] WHERE CampaignName=N'Nhắc Đánh Giá Sau Giao Hàng');
...
SELECT '  → Customers',                                   COUNT(*)           FROM Accounts WHERE RoleID=1              UNION ALL
SELECT '  → Staff/Admin/Merch',                           COUNT(*)           FROM Accounts WHERE RoleID>1              UNION ALL
...
SELECT 'WalletPinAttempts              [NEW]',            COUNT(*)           FROM WalletPinAttempts                    UNION ALL
...
SELECT 'Notification.CampaignApprovalLogs  [NEW]',        COUNT(*)           FROM [Notification].[CampaignApprovalLogs] UNION ALL
...
SELECT 'Recommendation.UserProductScores [NEW]',          COUNT(*)           FROM [Recommendation].[UserProductScores] UNION ALL
SELECT 'Recommendation.ItemSimilarities [NEW]',           COUNT(*)           FROM [Recommendation].[ItemSimilarities]  UNION ALL
