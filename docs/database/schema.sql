...
FROM sys.schemas
WHERE name = 'Notification') EXEC('CREATE SCHEMA [Notification]')
IF NOT EXISTS (SELECT *
FROM sys.schemas
WHERE name = 'Interaction')   EXEC('CREATE SCHEMA [Interaction]')
IF NOT EXISTS (SELECT *
FROM sys.schemas
WHERE name = 'Recommendation') EXEC('CREATE SCHEMA [Recommendation]')
IF NOT EXISTS (SELECT *
FROM sys.schemas
WHERE name = 'System')         EXEC('CREATE SCHEMA [System]')
GO
...
(
    ...
    -- thời điểm bắt đầu slot (UTC)
    [EndAt] DATETIME2(0) NOT NULL,
    -- thời điểm kết thúc slot (UTC)
    [Status] VARCHAR(20) NOT NULL DEFAULT 'Scheduled'
...
(
    ...
    -- FK → Promotions(PromotionID), NULL nếu không áp dụng KM
    [SlotProductID] INT NULL,
    -- FK → PromotionProductSlots(SlotProductID), NULL nếu không phải flash-sale
    [CreatedAt] DATETIME2(0) NOT NULL DEFAULT GETDATE(),
    [UpdatedAt] DATETIME2(0) NULL,
...
(
    ...
    [VoucherTarget] VARCHAR(20) NOT NULL CHECK ([VoucherTarget] IN ('ORDER_TOTAL', 'SHIPPING_FEE')),
...
BEGIN
    ...
    WHERE s.[BlogPostID] IN (SELECT [BlogPostID]
    FROM affected);
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO [BlogPostStats]
        ([BlogPostID], [LikeCount], [CommentCount], [UpdatedAt])
