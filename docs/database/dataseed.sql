...
/* =================================================================
   ...
   ─────────────────────────────────────────────────────────────────
   [FIX-01] Section 33 – StaffShiftCapacity:
            Trigger TR_WorkSchedules_CreateCapacity đã tự tạo row
            → Đổi sang UPDATE thay vì INSERT để tránh UNIQUE conflict
            → Thêm AccountID vào UPDATE vì schema v3.2 yêu cầu
   ...
   [NEW-43] Recommendation.Widgets  – cấu hình widget gợi ý
            Recommendation.TrendingProducts  – sản phẩm trending
            Recommendation.UserProductScores – điểm tương tác user
            Recommendation.ItemSimilarities  – độ tương đồng sp
   [NEW-44] WalletPinAttempts        – log thử PIN ví
   [NOTE]   BlogPostStats tự sinh qua trigger trg_BlogPost_InitStats
            → Không cần seed thủ công
================================================================= */
...
WHERE NOT EXISTS (
    SELECT 1
FROM [Notification].[UserPreferences] up
WHERE up.AccountID = a.AccountID);
GO
...
IF NOT EXISTS (SELECT 1
FROM [dbo].[UserBlockHistory])
BEGIN
    DECLARE @admBlk INT = (SELECT TOP 1
        AccountID
    FROM Accounts
    WHERE Email='admin@toyhouse.vn');
    DECLARE @br2    INT = (SELECT TOP 1
        BlockReasonID
    FROM BlockReasons
    WHERE Content=N'Dấu hiệu gian lận');
...
BEGIN
    DECLARE @pFlash1 INT = (SELECT TOP 1
        PromotionID
    FROM Promotions
    WHERE PromotionName=N'Siêu Flash Sale 5.5 – 48 Tiếng');
    DECLARE @pFlash2 INT = (SELECT TOP 1
        PromotionID
    FROM Promotions
    WHERE PromotionName=N'Flash Sale Ngày Gia Đình 28/6');
...
BEGIN
    DECLARE @pSale INT = (SELECT TOP 1
        PromotionID
    FROM Promotions
    WHERE PromotionName=N'Sale Hè Rực Rỡ 2026');
...
BEGIN
    ...
    WHERE StartAt='2026-05-04 02:00:00');
    DECLARE @s2 INT=(SELECT TOP 1
        TimeSlotID
    FROM PromotionTimeSlots
    WHERE StartAt='2026-05-04 13:00:00');
    DECLARE @s3 INT=(SELECT TOP 1
        TimeSlotID
    FROM PromotionTimeSlots
    WHERE StartAt='2026-05-05 02:00:00');
    DECLARE @s4 INT=(SELECT TOP 1
        TimeSlotID
    FROM PromotionTimeSlots
    WHERE StartAt='2026-05-05 13:00:00');
...
(
    ...
    ShipFee DECIMAL(10,0),
    VoucherDiscount DECIMAL(12,0),
...
VALUES
    ...
    ('bauchau.vu@gmail.com', 'ORD-2026-00003', 8, '2026-03-05 16:45:00', NULL, NULL, NULL, NULL, '2026-03-06 17:00:00',
        'BANK_TRANSFER', 'PENDING', NULL, 840000, 25000, 0, 865000, N'Khách chưa chuyển khoản sau 24h', NULL, NULL, N'Vũ Thị Bảo Châu', '0912001003', N'88 Nguyễn Trãi, P. Nhân Chính', 'W20301', 203, 2),
    ...
    ('mtuando@outlook.com', 'ORD-2026-00049', 7, '2026-02-18 09:00:00', '2026-02-18 11:00:00', '2026-02-19 09:00:00', '2026-02-20 14:00:00', '2026-02-21 08:00:00', NULL,
        'BANK_TRANSFER', 'PAID', '2026-02-18 09:00:00', 595000, 25000, 0, 620000, NULL, 'PAY-00049', NULL, N'Đỗ Minh Tuấn', '0912001004', N'34 Đinh Tiên Hoàng, P. Đa Kao', 'W10102', 101, 1),
...
VALUES
    ...
    ('ORD-2026-00004', 1, 1, 1290000),
    ('ORD-2026-00004', 29, 2, 88000),
    ('ORD-2026-00004', 30, 1, 175000),
    ('ORD-2026-00004', 6, 1, 145000),
    ('ORD-2026-00049', 22, 1, 595000),
    ('ORD-2026-00005', 20, 1, 359000),
    ('ORD-2026-00005', 17, 1, 840000),
...
INSERT INTO [dbo].[OrderStatusHistory]
    (OrderID,StatusID,Note,CreatedAt)
SELECT o.OrderID, 1, N'Đơn hàng mới', o.OrderDate
FROM Orders o
WHERE NOT EXISTS (SELECT 1
FROM OrderStatusHistory h
WHERE h.OrderID=o.OrderID AND h.StatusID=1);

INSERT INTO [dbo].[OrderStatusHistory]
    (OrderID,StatusID,Note,CreatedAt)
SELECT o.OrderID, 2, N'Xác nhận đơn', o.ConfirmedAt
FROM Orders o
WHERE o.ConfirmedAt IS NOT NULL
    AND NOT EXISTS (SELECT 1
    FROM OrderStatusHistory h
    WHERE h.OrderID=o.OrderID AND h.StatusID=2);

INSERT INTO [dbo].[OrderStatusHistory]
    (OrderID,StatusID,Note,CreatedAt)
SELECT o.OrderID, 3, N'Đang đóng gói', DATEADD(HOUR,2,o.ConfirmedAt)
FROM Orders o
WHERE o.ConfirmedAt IS NOT NULL AND o.StatusID IN (3,4,5,6,7)
    AND NOT EXISTS (SELECT 1
    FROM OrderStatusHistory h
    WHERE h.OrderID=o.OrderID AND h.StatusID=3);

INSERT INTO [dbo].[OrderStatusHistory]
    (OrderID,StatusID,Note,CreatedAt)
SELECT o.OrderID, 4, N'Bàn giao vận chuyển', o.ShippedAt
FROM Orders o
WHERE o.ShippedAt IS NOT NULL
    AND NOT EXISTS (SELECT 1
    FROM OrderStatusHistory h
    WHERE h.OrderID=o.OrderID AND h.StatusID=4);

INSERT INTO [dbo].[OrderStatusHistory]
    (OrderID,StatusID,Note,CreatedAt)
SELECT o.OrderID, 6, N'Giao hàng thành công', o.DeliveredAt
FROM Orders o
WHERE o.DeliveredAt IS NOT NULL
    AND NOT EXISTS (SELECT 1
    FROM OrderStatusHistory h
    WHERE h.OrderID=o.OrderID AND h.StatusID=6);

INSERT INTO [dbo].[OrderStatusHistory]
    (OrderID,StatusID,Note,CreatedAt)
SELECT o.OrderID, 7, N'Hoàn thành', o.CompletedAt
FROM Orders o
WHERE o.CompletedAt IS NOT NULL
    AND NOT EXISTS (SELECT 1
    FROM OrderStatusHistory h
    WHERE h.OrderID=o.OrderID AND h.StatusID=7);

INSERT INTO [dbo].[OrderStatusHistory]
    (OrderID,StatusID,Note,CreatedAt)
SELECT o.OrderID, 8, N'Đã hủy: '+ISNULL(o.CancelReason,N'Không rõ lý do'), o.CancelledAt
FROM Orders o
WHERE o.CancelledAt IS NOT NULL
    AND NOT EXISTS (SELECT 1
    FROM OrderStatusHistory h
    WHERE h.OrderID=o.OrderID AND h.StatusID=8);
GO
...
WHERE o.PaymentMethod='SE_PAY'
    AND NOT EXISTS (SELECT 1
    FROM PaymentGatewayTransactions pg
    WHERE pg.OrderID=o.OrderID);
GO
...
WHERE NOT EXISTS (SELECT 1
FROM Wallets w
WHERE w.AccountID=Accounts.AccountID);
...
WHERE wt.WalletID=w.WalletID AND wt.TxnType='TopUp');
...
SELECT
    ...
    20000000 - ISNULL((SELECT SUM(o2.TotalAmount)
    FROM Orders o2
...
INSERT INTO [dbo].[WalletPins]
    (WalletID,PinHash,IsActive,FailedAttempts,CreatedAt)
...
WHERE NOT EXISTS (
    SELECT 1
FROM ShippingStatusHistories sh
WHERE sh.ShippingTxId=sp.ShippingTransactionID);
GO

/* ══════════════════════════════════════════════════════════════
   SECTION 27 – ORDER REFUNDS
══════════════════════════════════════════════════════════════ */
PRINT N'[27] OrderRefundReasons, OrderRefunds...';
IF NOT EXISTS (SELECT 1
...
BEGIN
    DECLARE @refCust3 INT = (SELECT TOP 1
        AccountID
    FROM Accounts
    WHERE Email='bauchau.vu@gmail.com');
    DECLARE @refR1    INT = (SELECT TOP 1
        RefundReasonID
    FROM OrderRefundReasons
    WHERE Content=N'Sản phẩm bị lỗi từ nhà sản xuất');
    DECLARE @ord3     INT = (SELECT TOP 1
        OrderID
    FROM Orders
    WHERE OrderCode='ORD-2026-00003');
    ...
    VALUES
        (@ord3, @refR1, @refCust3, @refCust3, 'REF-2026-00001', 840000, 840000, 840000, 1, '2026-03-07 09:00:00');

    INSERT INTO [dbo].[RefundImages]
        (RefundID,ImageURL,CreatedAt)
    SELECT RefundID, 'https://picsum.photos/seed/refund-'+CAST(RefundID AS VARCHAR)+'-a/400/400', GETDATE()
    FROM OrderRefunds;
END
GO
...
BEGIN
    ...
    WHERE NOT EXISTS (
        SELECT 1
    FROM CartItems ci
    WHERE ci.CartID=c.CartID AND ci.ProductID=v.ProductID);
END

IF NOT EXISTS (SELECT 1
FROM [dbo].[Wishlists])
    INSERT INTO [dbo].[Wishlists]
    (AccountID,ProductID,CreatedAt)
...
IF NOT EXISTS (SELECT 1
FROM [dbo].[ProductFollowers])
    INSERT INTO [dbo].[ProductFollowers]
    (ProductID, AccountID, CreatedAt)
SELECT v.ProductID, a.AccountID, GETDATE()
-- Đã đảo lại đúng thứ tự
FROM Accounts a
    JOIN (VALUES
...
IF NOT EXISTS (SELECT 1
FROM [dbo].[CustomerChildren])
    INSERT INTO [dbo].[CustomerChildren]
    (AccountID,SexID,FullName,NickName,DOB,CreatedAt)
...
BEGIN
    ...
    INSERT INTO [dbo].[ReviewProductImages]
        (ReviewProductID,ImageURL,ModerationStatus,CreatedAt)
    SELECT ReviewID, 'https://picsum.photos/seed/rv'+CAST(ReviewID AS VARCHAR)+'-1/300/300', 'Approved', GETDATE()
    FROM ReviewProducts;
    INSERT INTO [dbo].[ReviewProductImages]
        (ReviewProductID,ImageURL,ModerationStatus,CreatedAt)
    SELECT ReviewID, 'https://picsum.photos/seed/rv'+CAST(ReviewID AS VARCHAR)+'-2/300/300', 'Approved', GETDATE()
    FROM ReviewProducts;

    INSERT INTO [dbo].[ReviewProductReactions]
        (ReviewProductID,AccountID,ReactionTypeID,CreatedAt)
    SELECT rp.ReviewID, a.AccountID, 1, GETDATE()
    FROM ReviewProducts rp
    CROSS JOIN (SELECT TOP 3
            AccountID
        FROM Accounts
        WHERE RoleID=1
    ...
    WHERE rp.AccountID <> a.AccountID
        AND NOT EXISTS (SELECT 1
        FROM ReviewProductReactions x
        WHERE x.ReviewProductID=rp.ReviewID AND x.AccountID=a.AccountID);
END
GO
...
/* ══════════════════════════════════════════════════════════════
   ...
   Trigger TR_WorkSchedules_CreateCapacity tự tạo row sau mỗi INSERT
   vào WorkSchedules. Seed v5.2 INSERT thủ công gây lỗi UNIQUE (ScheduleID).
   → Đổi sang UPDATE để cập nhật CurrentLoad cho các ca đã hoàn thành.
══════════════════════════════════════════════════════════════ */
...
FROM Accounts
WHERE Email='admin@toyhouse.vn');
DECLARE @stf1  INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='nhung.st@toyhouse.vn');
DECLARE @stf2  INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='dung.st@toyhouse.vn');
DECLARE @stf3  INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='thao.st@toyhouse.vn');
DECLARE @mc1   INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='bao.kho@toyhouse.vn');
DECLARE @mc2   INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='lan.kho@toyhouse.vn');
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@stf1 AND WorkDate=@dayWS AND ShiftTemplateID=1)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@stf1, 1, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@stf1 AND WorkDate=@dayWS AND ShiftTemplateID=2)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@stf1, 2, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@stf2 AND WorkDate=@dayWS AND ShiftTemplateID=2)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@stf2, 2, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@stf2 AND WorkDate=@dayWS AND ShiftTemplateID=3)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@stf2, 3, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@stf3 AND WorkDate=@dayWS AND ShiftTemplateID=1)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@stf3, 1, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@stf3 AND WorkDate=@dayWS AND ShiftTemplateID=3)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@stf3, 3, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@mc1 AND WorkDate=@dayWS AND ShiftTemplateID=2)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@mc1, 2, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@mc1 AND WorkDate=@dayWS AND ShiftTemplateID=3)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@mc1, 3, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@mc2 AND WorkDate=@dayWS AND ShiftTemplateID=1)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@mc2, 1, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
    IF NOT EXISTS (SELECT 1
    FROM WorkSchedules
    WHERE AccountID=@mc2 AND WorkDate=@dayWS AND ShiftTemplateID=2)
        INSERT INTO WorkSchedules
        (AccountID,ShiftTemplateID,WorkDate,Status,CreatedBy,CreatedAt)
    VALUES(@mc2, 2, @dayWS, @stWS, @admWS, DATEADD(DAY,-7,@dayWS));
...
DECLARE @schedStf INT=(SELECT TOP 1
    ScheduleID
FROM WorkSchedules
WHERE AccountID=(SELECT TOP 1
        AccountID
    FROM Accounts
    WHERE Email='nhung.st@toyhouse.vn')
    AND Status='Completed'
ORDER BY WorkDate ASC);
DECLARE @schedMrc INT=(SELECT TOP 1
    ScheduleID
FROM WorkSchedules
WHERE AccountID=(SELECT TOP 1
        AccountID
    ...
    AND Status='Completed'
ORDER BY WorkDate ASC);
DECLARE @admAsn INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='admin@toyhouse.vn');
DECLARE @stfAsn INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='nhung.st@toyhouse.vn');
DECLARE @mrcAsn INT=(SELECT TOP 1
    AccountID
FROM Accounts
WHERE Email='bao.kho@toyhouse.vn');
...
WHERE @schedStf IS NOT NULL
    ...
    WHERE oa.OrderID=o.OrderID AND oa.RoleID=3);
...
WHERE @schedMrc IS NOT NULL
    AND NOT EXISTS (SELECT 1
    FROM OrderAssignments oa
    WHERE oa.OrderID=o.OrderID AND oa.RoleID=4);

IF NOT EXISTS (SELECT 1
FROM [dbo].[OrderQueue])
    INSERT INTO [dbo].[OrderQueue]
    (OrderID,QueuedAt,Reason,IsResolved)
SELECT TOP 1
    o.OrderID, GETDATE(), 'NO_STAFF_ON_DUTY', 0
FROM Orders o
WHERE o.StatusID=1
...
BEGIN
    ...
    DECLARE @cmpS   INT = (SELECT TOP 1
        CampaignID
    FROM [Notification].[Campaigns]
    WHERE CampaignName=N'Thông Báo Sale Hè 2026');
    DECLARE @cmpV   INT = (SELECT TOP 1
        CampaignID
    FROM [Notification].[Campaigns]
    WHERE CampaignName=N'Gửi Voucher Welcome Khách Mới');
    DECLARE @cmpR   INT = (SELECT TOP 1
        CampaignID
    FROM [Notification].[Campaigns]
    WHERE CampaignName=N'Nhắc Đánh Giá Sau Giao Hàng');
...
PRINT N'';
PRINT N'================================================================';
PRINT N'  DataSeed v5.3 – HOÀN TẤT!';
PRINT N'================================================================';
PRINT N'';

    SELECT 'Roles'                               AS [Table], COUNT(*) AS [Rows]
    FROM Roles
UNION ALL
    SELECT 'Sexes', COUNT(*)
    FROM Sexes
...
UNION ALL
    ...
    WHERE RoleID=1
UNION ALL
    SELECT '  → Staff/Admin/Merch', COUNT(*)
    FROM Accounts
    WHERE RoleID>1
UNION ALL
...
UNION ALL
    ...
    FROM Products
UNION ALL
    SELECT 'ProductDetails', COUNT(*)
    FROM ProductDetails
UNION ALL
    SELECT 'ProductImages', COUNT(*)
    FROM ProductImages
UNION ALL
...
UNION ALL
    SELECT 'WalletPins', COUNT(*)
    FROM WalletPins
UNION ALL
    SELECT 'WalletPinAttempts              [NEW]', COUNT(*)
    FROM WalletPinAttempts
UNION ALL
    SELECT 'Cart', COUNT(*)
    FROM Cart
UNION ALL
...
UNION ALL
    ...
    FROM Wishlists
UNION ALL
    SELECT 'CustomerChildren', COUNT(*)
    FROM CustomerChildren
UNION ALL
    SELECT 'BlogPosts', COUNT(*)
    FROM BlogPosts
UNION ALL
    SELECT 'BlogPostStats (auto-trigger)   [NEW]', COUNT(*)
    FROM BlogPostStats
UNION ALL
    SELECT 'ReviewBlogs', COUNT(*)
    FROM ReviewBlogs
UNION ALL
    SELECT 'ReviewProducts', COUNT(*)
    FROM ReviewProducts
UNION ALL
    SELECT 'ReviewModerationLogs          [NEW]', COUNT(*)
    FROM ReviewModerationLogs
UNION ALL
    SELECT 'ShiftTemplates', COUNT(*)
    FROM ShiftTemplates
UNION ALL
    SELECT 'WorkSchedules', COUNT(*)
    FROM WorkSchedules
UNION ALL
    SELECT 'StaffShiftCapacity [FIX-01]', COUNT(*)
    FROM StaffShiftCapacity
UNION ALL
    SELECT 'OrderAssignments', COUNT(*)
    FROM OrderAssignments
UNION ALL
    SELECT 'Notification.Templates', COUNT(*)
    FROM [Notification].[Templates]
UNION ALL
    SELECT 'Notification.Campaigns', COUNT(*)
    FROM [Notification].[Campaigns]
UNION ALL
    SELECT 'Notification.CampaignApprovalLogs  [NEW]', COUNT(*)
    FROM [Notification].[CampaignApprovalLogs]
UNION ALL
    SELECT 'Notification.CampaignStats', COUNT(*)
    FROM [Notification].[CampaignStats]
UNION ALL
    SELECT 'Notification.Deliveries', COUNT(*)
    FROM [Notification].[Deliveries]
UNION ALL
    SELECT 'Notification.DeliveryActions   [NEW]', COUNT(*)
    FROM [Notification].[DeliveryActions]
UNION ALL
    SELECT 'Interaction.Events             [NEW]', COUNT(*)
    FROM [Interaction].[Events]
UNION ALL
    SELECT 'Recommendation.Widgets         [NEW]', COUNT(*)
    FROM [Recommendation].[Widgets]
UNION ALL
    SELECT 'Recommendation.TrendingProducts [NEW]', COUNT(*)
    FROM [Recommendation].[TrendingProducts]
UNION ALL
    SELECT 'Recommendation.UserProductScores [NEW]', COUNT(*)
    FROM [Recommendation].[UserProductScores]
UNION ALL
    SELECT 'Recommendation.ItemSimilarities [NEW]', COUNT(*)
    FROM [Recommendation].[ItemSimilarities]
UNION ALL
    SELECT 'System.BackgroundJobs', COUNT(*)
    FROM [System].[BackgroundJobs]
UNION ALL
    SELECT 'System.DomainEventOutbox', COUNT(*)
