...
_Withdrawal Flow via PayOS Payout_
...
| **ℹ️ Lưu ý:** Số dư khả dụng (AvailableBalance) = Balance - LockedBalance. View vw_WalletAvailableBalance được tạo sẵn để query nhanh. |
| :------------------------------------------------------------------------------------------------------------------------------------- |
...
| **🔒 Constraint:** Constraint UQ_SavedBankAccounts_AccountBin: mỗi cặp (AccountID, BankBin, AccountNumber) là unique. SP_Withdrawal_Lock tự upsert bảng này mỗi khi user rút. |
| :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| :------------: | :------------------: | :---------: | :----------------------------------------------------------------: |
|  **PENDING**   |      (khởi tạo)      |   SP_Lock   |              Vừa tạo, đã lock balance, chờ gọi PayOS               |
| **PROCESSING** |       PENDING        | App/Service |               Đã gọi PayOS API, chờ webhook callback               |
|  **SUCCESS**   | PENDING / PROCESSING |  SP_Commit  |     Webhook SUCCESS: tiền đã chuyển thành công, Balance bị trừ     |
|   **FAILED**   | PENDING / PROCESSING | SP_Rollback | PayOS thất bại / timeout: LockedBalance được mở, Balance không đổi |
...
Migration cung cấp 3 SP tương ứng 3 giai đoạn của luồng rút tiền. Tất cả đều sử dụng SET XACT_ABORT ON và BEGIN/COMMIT TRANSACTION.

## **3.1. SP_Withdrawal_Lock — Khởi tạo lệnh rút**
...
1. Sinh ReferenceId = "WD{AccountID}{timestamp_ms}".
...
1. COMMIT — trigger TR_WithdrawalRequests_StatusHistory tự ghi lịch sử.

## **3.2. SP_Withdrawal_Commit — Xác nhận thành công**
...
## **3.3. SP_Withdrawal_Rollback — Hoàn lại khi thất bại**
...
1. Frontend validate: kiểm tra Amount >= 10,000 và không vượt AvailableBalance (query từ view vw_WalletAvailableBalance).
...
> **⚠ Quan trọng:** PayOS Payout (Chi hộ) **không có webhook callback**. Mục **Thu (thanh toán đơn hàng)** mới có webhook. Mục **Chi (payout)** chỉ có REST GET để tra cứu trạng thái. `PayosPayoutWebhookController` trong codebase là dead code và không nhận callback từ PayOS.

Sau khi `POST /v1/payouts` trả về `approvalState: PROCESSING`, `WithdrawalPayoutPollJob` (Worker) thực hiện:

1. Mỗi 2 phút, query `WithdrawalRequests` WHERE `Status = PROCESSING` AND `PayosPayoutId IS NOT NULL`.
2. Với mỗi lệnh, gọi `GET /v1/payouts/{payoutId}` — headers `x-api-key` + `x-client-id`.
3. Map `approvalState`:
   - `PROCESSING` → bỏ qua, poll lần sau.
   - `SUCCEEDED` → gọi `CommitAsync` (trừ Balance, tạo WalletTransaction), gửi push notification thành công.
   - `FAILED` / `CANCELLED` / `REJECTED` → gọi `RollbackAsync` (mở LockedBalance), gửi push notification thất bại.
4. Ghi `WithdrawalStatusHistory` với `Source = JOB`.

`WithdrawalTimeoutJob` (chạy mỗi 5 phút) là lớp an toàn cuối: với lệnh PROCESSING quá `TimeoutMinutes` (mặc định 30 phút), nó thử poll PayOS một lần nữa trước khi rollback force.
...
1. Với mỗi lệnh quá hạn: gọi SP_Withdrawal_Rollback với FailReason = "Timeout after 30 minutes".
1. Ghi log vào WithdrawalStatusHistory với Source = "JOB".
1. Nếu SP trả ErrorCode = 3 (đã SUCCESS) → bỏ qua, không xử lý. SP_Withdrawal_Rollback đã có guard kiểm tra Status trước khi update, nên dù webhook SUCCESS và job timeout chạy đồng thời, lệnh rút sẽ không bị rollback nhầm — chỉ 1 trong 2 được commit thực sự.

| **⚡ Performance:** Index IX_WithdrawalRequests_Pending đã được tạo sẵn (Status IN PENDING/PROCESSING, ORDER BY CreatedAt ASC) để job query hiệu quả. |
| :---------------------------------------------------------------------------------------------------------------------------------------------------- |
...
|  **2**   |      SP_Withdrawal_Lock chạy      |                                                                 Wallets.LockedBalance += Amount; INSERT WithdrawalRequests (PENDING)                                                                  |
|    3     |       Gọi PayOS Payout API        |                                                                                WithdrawalRequests.Status = PROCESSING                                                                                 |
|  **4A**  |   Nhận webhook SUCCESS từ PayOS   |                                                            SP_Commit: Balance -= Amount, LockedBalance -= Amount, INSERT WalletTransaction                                                            |
|  **4B**  |   Nhận webhook FAILED / timeout   |                                                                       SP_Rollback: LockedBalance -= Amount (Balance không đổi)                                                                        |
|    5     | Gửi notification kết quả cho user | INSERT Notification.Deliveries — trigger từ webhook handler sau khi SP_Commit / SP_Rollback thành công. Nếu notification fail: không rollback lệnh rút, ghi log lỗi và để background job retry riêng. |

## **4.5. PayOS Payout API — Tạo lệnh chi đơn**

Backend gọi API này sau khi SP_Withdrawal_Lock chạy thành công (Status = PENDING). Đây là bước chuyển tiền thực sự qua cổng PayOS Payout.
...
| **Header: x-idempotency-key** | Điền vào @ReferenceId trả về từ SP_Withdrawal_Lock. Đảm bảo idempotency: gọi lại cùng key không tạo thêm giao dịch mới. |
...
| :------------------ | :------------------ | :------------------------------------------------------------------------------------------- |
| **referenceId**     | string / required   | Idempotency key. Điền vào @ReferenceId OUTPUT từ SP_Withdrawal_Lock. VD: WD-42-1720000000000 |
...
| **429**  | Too Many Requests — Gọi API quá nhiều trong thời gian ngắn. Backend implement exponential backoff: retry tối đa 3 lần, delay 2s → 4s → 8s. Sau 3 lần thất bại: gọi SP_Withdrawal_Rollback ngay, không chờ background job. |
| **500**  | PayOS Internal Error — Ghi log PayosRawResponse để debug. Retry tối đa 3 lần với exponential backoff (2s → 4s → 8s). Nếu vẫn lỗi sau 3 lần: gọi SP_Withdrawal_Rollback để mở lock ngay, không chờ background job 30 phút. |
...
**Giải pháp khuyến nghị —** Sử dụng BankLookup API để tra tên chủ TK trước khi user xác nhận. Frontend hiển thị tên cho user xác nhận, sau đó backend truyền @ToAccountName vào SP_Withdrawal_Lock để lưu. PayOS sẽ xác minh lại nội bộ khi chuyển tiền thực tế.

**4.4.1. BankLookup API — Tra cứu tên chủ tài khoản**

**Bước 1 — Lấy danh sách ngân hàng:** GET https://api.banklookup.net/bank/list — không cần auth. Response trả về mảng ngân hàng với các field: code, bin, short_name, lookup_supported. Cache kết quả (IMemoryCache, TTL 24h). Chỉ hiển thị ngân hàng có lookup_supported = 1 trong dropdown frontend.
...
{ "code": 422, "success": false, "msg": "NOT_FOUND", "data": null } // TK không tồn tại hoặc ngân hàng không hỗ trợ → chặn luồng rút
...
| payout-reference-id | Điền @ReferenceId trả về từ SP_Withdrawal_Lock. VD: WD-42-1720000000000        |
...
## **4.7. PayOS Payout — Tra cứu trạng thái lệnh chi (GET /v1/payouts)**

> **⚠ Cảnh báo thiết kế:** PayOS Payout (Chi hộ) KHÔNG có webhook. Phần `confirm-webhook` và Webhook Callback chỉ áp dụng cho luồng Thu (Link thanh toán đơn hàng). Để biết kết quả lệnh chi, dùng GET polling.

### **4.7.1. GET /v1/payouts/{payoutId}**

| **Thuộc tính** | **Giá trị** |
| :------------- | :---------- |
| **Method & URL** | **GET** https://api-merchant.payos.vn/v1/payouts/{payoutId} |
| **Header: x-api-key** | API Key kênh Chi hộ |
| **Header: x-client-id** | Client ID kênh Chi hộ |

**Response 200 — Các field quan trọng**

| **Field** | **Kiểu** | **Mô tả** |
| :-------- | :------- | :-------- |
| data.id | string | PayOS Payout ID (= WithdrawalRequests.PayosPayoutId) |
| data.approvalState | string | `PROCESSING` / `SUCCEEDED` / `FAILED` |
| data.transactions[0].id | string | Transaction ID lưu vào WithdrawalRequests.PayosTransactionId |
| data.transactions[0].state | string | Trạng thái transaction chi tiết |

### **4.7.2. Logic mapping trong WithdrawalPayOsSyncService**

```
approvalState = SUCCEEDED  → CommitAsync + notification success
approvalState = FAILED / CANCELLED / REJECTED → RollbackAsync + notification failed
approvalState = PROCESSING → bỏ qua, poll lần sau
```

### **4.7.3. GET /v1/payouts?referenceId={referenceId}**

Backup query theo merchant referenceId khi cần tra cứu mà không có `payoutId`:

```
GET https://api-merchant.payos.vn/v1/payouts?referenceId=WD12345678
```
...
|              **Constraint**               |                                        **Quy tắc**                                         |
| :---------------------------------------: | :----------------------------------------------------------------------------------------: |
|   **CK_Wallets_LockedNotExceedBalance**   |               LockedBalance <= Balance (không thể lock nhiều hơn số dư thực)               |
| **CK_WithdrawalRequests_TxnConsistency**  |    WalletTransactionID IS NOT NULL chỉ khi Status = SUCCESS và CompletedAt IS NOT NULL     |
| **CK_WithdrawalRequests_FailConsistency** |                    FailReason IS NOT NULL bắt buộc khi Status = FAILED                     |
|     **CK_WithdrawalRequests_Amount**      |                             Amount > 0 (không cho rút 0 đồng)                              |
|      **CHECK Amount >= 10000 (SP)**       |              SP_Withdrawal_Lock validate Amount >= 10,000 VND trước khi lock               |
|     **CK_WalletTransactions_TxnType**     | Sau migration: TxnType IN (TopUp, Payment, Refund, Withdrawal, LockReserve, UnlockRelease) |
...
- SET XACT_ABORT ON đảm bảo rollback tự động nếu có lỗi.
...
- Kiểm tra constraint CK_WalletTransactions_TxnType: drop constraint cũ tự động bằng dynamic SQL đã có trong migration.
- Verify view vw_WalletAvailableBalance đã tạo thành công.
...
| SELECT AccountID, Balance, LockedBalance, AvailableBalance, Status FROM vw_WalletAvailableBalance WHERE AccountID = @AccountID |
| :----------------------------------------------------------------------------------------------------------------------------- |
